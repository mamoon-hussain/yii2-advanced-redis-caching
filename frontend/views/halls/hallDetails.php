...
//                'events' => Yii::$app->urlManager->createUrl('/halls/jsoncalendar?id='.$model->id),
            'events' => new \yii\web\JsExpression('{
                            url: \''.Yii::$app->urlManager->createUrl("/halls/jsoncalendar?id=$model->id").'\',
                            method: \'post\',
                            data: function() {
                                  var request_dates = "";
                                  $(".request_dates").each(function(){
                                  request_dates = request_dates+","+$(this).val()
                                  });
                                  return {
                                    chosen_dates: request_dates
                                  };
                                },
                            failure: function() {
                ...
                'eventClick' => new \yii\web\JsExpression("function(event, jsEvent, view) {
                    if($(this).hasClass('calender_day_chosen')){
                        $(this).removeClass('calender_day_chosen');
                        $('#request_dates_input_'+event.id).remove();
                    } else {
                        $(this).addClass('calender_day_chosen');
                        $('#request_dates_div').append('<input class=\"request_dates\" id=\"request_dates_input_'+event.id+'\" value=\"'+event.id+'\" name=\"Request[dates][]\" style=\"display: none\">');
                    }
                    ...
                    var period = '';
                    $('.request_dates').each(function() {
                        period = period + ',' + $(this).val();
                    });
                    $.get(window.url  + '/site/get-hall-price', {h: hall, p:period}, function(data){
                        var data = $.parseJSON(data);
                        $(\"#summery\").html('<div class=\"alert alert-success\" role=\"alert\">' + data.text + '</div>');
                    });
                }"),
            ));
            ?>

            <div id="request_dates_div" class="hidden">
